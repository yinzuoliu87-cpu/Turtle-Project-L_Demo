...
const ALL_PETS = [
  ...
  { id:'stone',     name:'石头龟',   emoji:'🪨🐢',    rarity:'C',   hp:380,  atk:36,  def:18, mr:15,crit:0.25,
    ...
    passive:{ type:'stoneWall', name:'坚壁', defGainAtkPct:0.07, maxDefInitPct:50, reflectBase:5, reflectPerDef:1, reflectPerMr:0.5,
              brief:'石头龟越战越硬。每回合永久 <span class="val-def">护甲</span> +<span class="val-atk">0.07%攻击力</span>（上限为<span class="val-atk">50%开局护甲</span>）；受伤时反弹部分伤害给攻击者（<span class="val-atk">5%</span> + <span class="val-def">1%×护甲</span> + <span class="val-magic">0.5%×魔抗</span>）。',
              desc:'石头龟越战越硬。\n\n每回合永久增加 <span class="val-def">护甲</span> <span class="val-atk">+{D:ATK*0.0007}</span>（上限 <span class="val-atk">50%开局护甲</span>，已叠加 <span class="val-atk">+{stoneDefGained}</span>）\n\n受到伤害时反弹：\n5% + 1%×<span class="val-def">护甲</span>({DEF}) + 0.5%×<span class="val-magic">魔抗</span>({MR}) = <span class="val-atk">{N:5+DEF+MR*0.5}%</span> 伤害' },
    skillPool:[
