...
const ALL_PETS = [
  ...
  { id:'cyber',     name:'赛博龟',   emoji:'🤖🐢',    rarity:'S',   hp:330,  atk:43,  def:13, spd:15, crit:0.25,
    ...
    passive:{ type:'cyberDrone', droneScale:0.2, droneMaxAge:5, maxDrones:10, mechHpPer:30, mechAtkPer:5,
              desc:'每回合+1浮游炮（上限10），每个每回合造成 20%×(攻击力={ATK}) = {N:ATK*0.2} 普通伤害；阵亡时组装为机甲' },
    skills:[
      ...
      { name:'部署', type:'cyberDeploy', hits:1, power:0, pierce:0, cd:2,
        brief:'赛博龟立即部署1个浮游炮（每回合{N:ATK*0.2}普通伤害，上限10）',
        detail:'赛博龟立即生成1个浮游炮。\n每个浮游炮每回合对随机敌人造成 20%×(攻击力={ATK}) = {N:ATK*0.2} 普通伤害。\n最多持有10个。冷却2回合。' },
    ]},
