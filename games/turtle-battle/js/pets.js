...
const PET_basic = { id:'basic',     name:'小龟',     emoji:'🐢',      rarity:'C',   hp:350,  atk:40,  def:14, mr:13,crit:0.25,
    ...
    skillPool:[
      ...
      { name:'龟派气波', type:'basicChiWave', hits:3, power:0, pierce:0, cd:4, atkScale:2.5,
        ...
        brief:'自增 <span class="val-atk">+25%暴击</span> / <span class="val-atk">+20%爆伤</span> / <span class="val-heal">+10%吸血</span> / <span class="val-atk">+{N:0.1*ATK}穿甲</span> 1回合，气波穿透目标所在横排（同高度的前+后位置，最多2敌），每人3段共（{N:2.5*ATK}）物理伤害',
        detail:'小龟获得 1 回合增益：\n· <span class="val-atk">+25%</span> 暴击率\n· <span class="val-atk">+20%</span> 暴击伤害\n· <span class="val-heal">+10%</span> 生命偷取\n· <span class="val-atk">+</span>（10%×攻击力 = {N:0.1*ATK}）护甲穿透\n\n选择敌方一个目标（前排优先），镜头推近 → 蓄力 → 气波缓缓飞出，穿透目标所在<b>横排</b>（同高度的前+后位置，最多2个敌人）：\n· 每个被气波击中的敌人被击飞空中，3 段连击共（250%×攻击力({ATK}) = {N:2.5*ATK}）物理伤害\n· 前排先被击飞，气波继续前行命中后排\n\n冷却{cd}回合。' },
...
const PET_angel = { id:'angel',     name:'天使龟',   emoji:'😇🐢',    rarity:'B',   hp:340,  atk:41,  def:13, mr:16,crit:0.25,
    ...
    skillPool:[
      ...
      { name:'平等', type:'angelEquality', dmgType:'physical', hits:2, power:0, pierce:0, cd:5, normalScale:0.8, pierceScale:0.8,
        antiHighRarity:['A','S','SS','SSS'], forceCrit:true, healPctOfDmg:10,
        brief:'天使龟两段：（{N:0.8*ATK}）物理 +（{T:0.8*ATK}）真实，A级以上必暴击+10%吸血',
        detail:'天使龟打击两段：\n· 第一段（80%×攻击力 = {N:0.8*ATK}）物理伤害\n· 第二段（80%×攻击力 = {T:0.8*ATK}）真实伤害\n\n目标为A级及以上时必定暴击，回复总伤害的10%HP。\n冷却{cd}回合。' },
...
const PET_ninja = { id:'ninja',     name:'忍者龟',   emoji:'🥷🐢',    rarity:'B',   hp:329,  atk:47,  def:9,  mr:7,crit:0.25,
    img:'assets/pets/animations/ninja/idle.png',
    sprite:{frames:6, frameW:64, frameH:64, duration:600},
    ...
    knockupAnim:{ src:'assets/pets/animations/ninja/knockup.png', frames:2, frameW:64, frameH:64, airborneMs:600, lyingMs:1100 },
...
const PET_bubble = { id:'bubble',    name:'泡泡龟',   emoji:'🫧🐢',    rarity:'A',   hp:350,  atk:39,  def:18, mr:19,crit:0.25,
    ...
    skillPool:[
      ...
      { name:'泡泡束缚', type:'bubbleBind', hits:1, power:0, pierce:0, cd:8, duration:8, lossCap:30,
        brief:'束缚敌方8回合，期间每受到一段伤害，护甲和魔抗各 <span class="val-atk">-X</span>（1~5级 X=1，6~10级 X=2），单次束缚累计削减上限 <span class="val-atk">30</span>',
        detail:'泡泡龟将敌方单体束缚8回合。\n束缚期间每受到一段伤害，目标的护甲与魔抗各减少 X 点（按泡泡龟自身等级：1~5级 X=1，6~10级 X=2）。\n累计削减各上限 <span class="val-atk">30</span>（达上限后此束缚不再继续削减）。\n减益为永久衰减，不会随束缚结束恢复。\n冷却{cd}回合。' },
      { name:'泡泡爆破', type:'bubbleBurst', dmgType:'magic', hits:1, power:0, pierce:0, cd:4, bubbleConsumePct:100, bubbleMagicScale:0.8, atkScale:0.8,
        brief:'泡泡龟消耗100%储存泡泡值，对目标所在竖排（前排3个 或 后排3个）每个敌人造成（0.8×消耗值）魔法 +（{N:0.8*ATK}）物理',
        detail:'泡泡龟消耗当前所有储存泡泡值（100%），在目标所在<b>竖排</b>（同前/同后的3个位置）两侧召唤2个泡泡向中间夹击。\n对该竖排每个敌人（最多3个：F1+F2+F3 或 B1+B2+B3）造成：\n· 80% × 消耗的泡泡值 的<span class="val-magic">魔法伤害</span>\n· 80% × 攻击力({ATK}) = {N:0.8*ATK} 的<span class="val-direct">物理伤害</span>\n\n两个数字独立跳出（蓝魔 + 红物理）。\n冷却{cd}回合。' },
      { name:'治愈泡泡', type:'bubbleHeal', hits:1, power:0, pierce:0, cd:4, isAlly:true, healAtkPct:120, healHpPct:10, splashPct:25,
...
const PET_cyber = { id:'cyber',     name:'赛博龟',   emoji:'🤖🐢',    rarity:'S',   hp:360,  atk:47,  def:14, mr:13,crit:0.25,
    ...
    skillPool:[
      ...
      { name:'能量大炮', type:'cyberBeam', hits:2, power:0, pierce:0, cd:3, atkScale:0.5, droneTrueScale:0.10, droneTrueScaleEnhanced:0.07,
        brief:'赛博龟跳到目标横排，对横排两个敌人（同高度的前+后位置）各造成2段共（{N:ATK}）物理 + 2段共（{T:0.10*ATK}）真实/浮游炮（强化后 {T:0.07*ATK}/炮台）',
        detail:'赛博龟跃至目标所在<b>横排</b>（同高度的前+后位置），从自身向屏幕边缘打出激光。\n对该横排每个敌人（最多2个：1个前位 + 1个后位）造成：\n· 2段物理伤害，每段 50%×攻击力({ATK}) = {N:0.5*ATK}（共 {N:ATK}）。\n· 2段真实伤害，每段 5%×攻击力 × 浮游炮数({B:droneCount}) = {T:0.05*ATK*droneCount}（共 {T:0.10*ATK*droneCount}）。\n\n若已装备「强化浮游炮」，真实倍率改为 3.5%/段（共7%）/炮台。\n\n4个数字独立跳出：2红物理 + 2金真实。\n冷却{cd}回合。' },
      { name:'部署', type:'cyberDeploy', hits:1, power:0, pierce:0, cd:2, deployCount:3,
        brief:'赛博龟立即部署 <span class="val-atk">3</span> 个浮游炮（每回合（{N:ATK*0.18}）物理伤害，上限10）',
        detail:'赛博龟立即生成 <span class="val-atk">3</span> 个浮游炮（受上限限制）。\n每个浮游炮每回合对随机敌人造成（18%×攻击力({ATK}) = {N:ATK*0.18}）物理伤害。\n最多持有10个（强化后20个）。\n冷却{cd}回合。' },
      { name:'强化浮游炮', type:'cyberEnhancedDrone', passiveSkill:true, hits:0, power:0, pierce:0, cd:0,
        brief:'浮游炮上限20个，每回合获得2个，每个造成12%ATK物理伤害。机甲额外获得(3×浮游炮数)护甲和魔抗',
        detail:'· 浮游炮上限 10→20\n· 每回合生成 1→2 个\n· 每个浮游炮伤害 18%→12% ATK\n· 机甲变身后额外获得（3×浮游炮数量）护甲和魔抗' },
