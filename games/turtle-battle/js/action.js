...
async function processPendingMechTransforms() {
  for (const ff of allFighters) {
    if (ff._pendingMech) {
      ...
      // JS particle convergence (drone parts flying inward) — keep this on top
      // of the sprite for extra visual density.
...
async function executeAction(action) {
  ...
  if (_handledByRegistry) {
    ...
    if (target && target.alive) {
      ...
      spawnFloatingNum(getFighterElId(target), `${dmg}`, 'magic-dmg', 0, 0, {atkSide:f.side, amount:dmg});
