...
async function executeAction(action) {
  ...
  // ── AOE Ally Heals ──
    ...
    const target = allFighters[action.targetId];
    if (target && target.alive) { await doDamage(f, target, skill); if (skill.atkDown) { const atkLoss = Math.round(target.baseAtk * skill.atkDown.pct / 100); target.buffs.push({ type:'atkDown', value:atkLoss, turns:skill.atkDown.turns }); spawnFloatingNum(getFighterElId(target), `-${atkLoss}攻`, 'debuff-num', 200, 0); } if (skill.spdDown) { target.buffs.push({ type:'spdDown', value:skill.spdDown.pct, turns:skill.spdDown.turns }); spawnFloatingNum(getFighterElId(target), `减速`, 'debuff-num', 300, 0); } recalcStats(); renderStatusIcons(target); updateFighterStats(target, getFighterElId(target)); }
  } else if (skill.type === 'lineInkBomb') {
    const enemies = getAliveEnemiesWithSummons(f.side);
    for (const enemy of enemies) { for (let h = 0; h < (skill.hits||3); h++) { const dmg = Math.round(f.atk * (skill.atkScale||0.3)); applyRawDmg(f, enemy, dmg, false, false, 'physical'); spawnFloatingNum(getFighterElId(enemy), `-${dmg}`, 'direct-dmg', h * 60, 0, {atkSide:f.side, amount:dmg}); if (battleOver) break; } updateHpBar(enemy, getFighterElId(enemy)); if (skill.inkStacks) { enemy._inkStacks = (enemy._inkStacks||0) + skill.inkStacks; const maxInk = (f._passiveSkills && f._passiveSkills.some(p => p.type === 'lineRapid')) ? 7 : 5; enemy._inkStacks = Math.min(enemy._inkStacks, maxInk); spawnFloatingNum(getFighterElId(enemy), `+${skill.inkStacks}墨迹`, 'debuff-num', 200, 0); renderStatusIcons(enemy); } await triggerOnHitEffects(f, enemy, Math.round(f.atk * (skill.atkScale||0.3) * (skill.hits||3))); if (battleOver) break; }
