...
const EQUIP_POOL = [
  ...
  { id:'e_carapace', name:'珊瑚硬壳', icon:'equip/dungeon-armor.png', desc:'护甲/魔抗 +15%', apply(f) { f.baseDef = Math.round(f.baseDef * 1.15); f.def = f.baseDef; f.baseMr = Math.round((f.baseMr||f.baseDef) * 1.15); f.mr = f.baseMr; } },
  ...
  { id:'e_piercer', name:'双穿珊瑚刺', icon:'equip/dungeon-spike.png', desc:'物理/魔法穿透 +6', apply(f) { f.armorPen += 6; f.magicPen = (f.magicPen||0) + 6; } },
  // Special effect (12)
  ...
  { id:'e_ripple', name:'潮汐涟漪', icon:'equip/dungeon-shell.png', desc:'单体攻击溅射30%伤害到1个随机其他敌人', apply(f) { f._equipSplash = 30; } },
  { id:'e_thunder_shell', name:'雷鸣贝壳', icon:'equip/dungeon-crystal.png', desc:'每3次技能命中引爆80%ATK魔法AoE', apply(f) { f._equipThunderShell = true; f._thunderShellStacks = 0; } },
];
